...
function valueFor(name){
  ...
  const namedInputs=Array.from(document.querySelectorAll(`[name="${name}"]`));
  const choiceInputs=namedInputs.filter(item=>item.type==='checkbox' || item.type==='radio');
  if(choiceInputs.length){
    const checked=choiceInputs.filter(item=>item.checked).map(item=>item.value);
...
function checkboxAnswerFor(node){
  const boxes=Array.from(node.querySelectorAll('input[type="checkbox"][name]'));
  if(!boxes.length)return null;
  const checked=boxes.filter(box=>box.checked).map(box=>box.value.trim()).filter(Boolean);
  const answerSource=node.querySelector('[data-answer]')?.dataset.answer||node.dataset.answer||'';
  if(!answerSource)return {ready:checked.length>0,ok:false,checked,expected:null};
  const expected=answerSource.split(';').map(item=>item.trim()).filter(Boolean);
  const ok=expected.length===checked.length&&expected.every(item=>checked.includes(item));
  return {ready:checked.length>0,ok,checked,expected};
}
...
function checkQuestion(node){
  ...
  const checkbox=checkboxAnswerFor(node);
  let ok=false;
  let explanation='';
  if(textField){
    ...
    ok=checkbox.ready&&checkbox.ok;
    explanation=node.querySelector('input[type="checkbox"][data-answer]')?.dataset.explain||node.dataset.explain||'';
...
function answeredFor(node,answer){
  ...
  if(node.querySelector('input[type="checkbox"]'))return checkboxAnswerFor(node)?.ready||Boolean(node.querySelector('input[type="checkbox"]:checked'));
...
function statusFor(node){
  ...
  if(node.querySelector('input[type="checkbox"]')){
    const info=checkboxAnswerFor(node);
    if(!info?.ready)return '未作答';
    return info.ok?'✅ 正确':'❌ 待修正';
  }
